...
let visitor = {
  ...
  ExportDefaultDeclaration (path, ref = { opts: {} }) {
    ...
    console.log('manifest', ref.opts.manifest && ref.opts.manifest.gitCommit)
    ...
    if (type === 'ObjectExpression' || type === 'Identifier') {
      ...
      let callExpression = types.expressionStatement(
        types.callExpression(types.identifier('namespace'), [types.stringLiteral(moduleRefer.replace(/\.\w+$/, ''))])
      )
      // 1.0.3 20180806
      // let exportDeclaration = types.exportDefaultDeclaration(
      //   types.assignmentExpression('=', buildMemberExprssion(moduleRefer), types.cloneDeep(declaration))
      // )
      // ^1.1.0 20190123
      let exportDeclaration = types.exportDefaultDeclaration(
        types.assignmentExpression(
          ...
          types.callExpression(
            types.memberExpression(types.identifier('Object'), types.identifier('assignIn'), false),
